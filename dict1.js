...
var database = [
    ...
    },
    {
        ...
        "others": "信仰使者：\n每次部署阶段，召唤一名<span style='font-weight:bold;color:#ffffff;'>近战小兵</span>至普瑞蕾斯的兵线。",
        "color": "blue"
    },
    {
        "name": "殁境神蚀者",
    ...
    },
    {
        ...
        "others": "忽悠：\n将米波移至友方<span style='font-weight:bold;color:#ffffff;'>米波</span>的兵线，对新的敌方近邻造成2伤害。",
        "color": "blue"
    },
    {
        "name": "剧毒术士",
    ...
    },
    {
        ...
        "others": "野性呼唤：\n召唤一头<span style='font-weight:bold;color:#ffffff;'>忠诚野兽</span>。",
        "color": "red"
    },
    {
        "name": "潮汐猎人",
        "pic": "[img]./mon_201812/02/-659kbQ5-dvfrZ1rT1kSem-oq.png.thumb_s.jpg[/img]",
        "others": "毁灭：\n在本回合晕眩潮汐猎人的敌方近邻，其他敌人也有50%几率被晕眩。",
        "color": "red"
    },
    {
        "name": "伐木机",
...
function press(){
    ...
    for (var i in database){
        ...
        if (findthis != 1){
            ...
            if (color === 'black'){
                content = replacewithoutdict(content, data.name, "[dict][{0}][/dict]".format(data.name));
            }
            else{
                content = replacewithoutdict(content, data.name, "[color={0}][dict][{1}][/dict][/color]".format(color, data.name));
            }
            if (data.others != ''){
                if (color === 'black'){
                    ...
                    content = replacewithoutdict(content, "【占位符】", "[color={0}][dict][{1}]{2}\n{3}[/dict][/color]".format(data.name, data.pic, deletespan(data.others)));
                }
            }
            else{
                if (color === 'black'){
